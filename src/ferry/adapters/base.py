...
Every tool Ferry supports implements :class:`Adapter`; the CLI never touches
a tool's storage directly.

All four adapters began as stubs that reported themselves as not installed —
the wiring existed so the interface could be built and tested before any
tool-specific logic was written. The stubs reported honestly rather than
inventing conversation counts.
"""
...
class Adapter(ABC):
    ...
    def import_(self, bundle_dir: Path, options: ImportOptions) -> Iterator[ImportEvent]:
        ...
        that is what makes cross-tool migration possible.
        """
...
class NotImplementedAdapter(Adapter):
    ...
    screen can render truthfully without inventing data. Each of these is
    replaced by a real implementation when that tool's adapter is built.
    """
...
REGISTRY: dict[str, Adapter] = {
    "claude-code": NotImplementedAdapter("claude-code", "Claude Code", "a later release"),
    "codex": NotImplementedAdapter("codex", "OpenAI Codex", "a later release"),
    "copilot": NotImplementedAdapter("copilot", "GitHub Copilot Chat", "a later release"),
    "antigravity": NotImplementedAdapter("antigravity", "Antigravity", "a later release"),
}
